...
const DisplaySongs = (props) => {
  ...
  return (
    <ListItem alignItems="flex-start" style={props.style}>
      ...
      />
      {searched ? (
        mapping[song.id].status === "Found" ? (
          <a href={mapping[song.id].song.external_urls.spotify}>
            open in spotify
          </a>
        ) : (
          mapping[song.id].status
        )
      ) : (
        "no search"
      )}
    ...
    <Card className={classes.root} raised={true} ref={cardRef}>
      ...
      <CardActions>
        <Button
          ...
          color="primary"
          onClick={() => {
            setNumItemsDisplayed((prev) => {
              return prev * 2 > numItems ? numItems : prev * 2;
            });
          }}
